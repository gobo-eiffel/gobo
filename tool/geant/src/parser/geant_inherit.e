...
feature -- Status report
	...
	validate_parent_selects
		...
		do
			if attached project.inherit_clause as l_inherit_clause then
				...
				from a_parent_cursor.start until a_parent_cursor.after loop
					...
					if attached a_parent.parent_project as l_parent_project then
						a_selected_cursor := l_parent_project.selected_targets.new_cursor
						from a_selected_cursor.start until a_selected_cursor.after loop
							...
							from a_parent_cursor2.start until a_parent_cursor2.after loop
								...
								if a_parent /= a_parent2 and then attached a_parent2.parent_project as l_parent_project_2 then
									a_selected_cursor2 := l_parent_project_2.selected_targets.new_cursor
	...
	sort_out_selected_targets
		...
		do
			if attached project.inherit_clause as l_inherit_clause and attached project.targets as l_project_targets then
				...
				from a_parent_cursor.start until a_parent_cursor.after loop
					...
					if attached a_parent.parent_project as l_parent_project then
						...
						from a_selected_cursor.start until a_selected_cursor.after loop
							a_selected_target_name := a_selected_cursor.key
							if not l_project_targets.has (a_selected_target_name) then
								exit_application (1, <<"%NLOAD ERROR:%N", "  project '", project.name,
									"' selected target `", a_selected_target_name, "' does not exist.">>)
							end
							a_selected_target := l_project_targets.item (a_selected_target_name)
							...
							loop
								a_conflict_found := False
								a_target_cursor := l_project_targets.new_cursor
								from a_target_cursor.finish until a_target_cursor.before loop
									...
									if a_target /= a_selected_target then
										if a_selected_target.conflicts_with (a_target) then
											...
											if attached a_target.precursor_target as l_precursor_target then
												l_precursor_target.set_redefining_target (a_selected_target)
											end
											l_project_targets.remove (a_target_name)
										end
									end
	...
	check_targets_for_conflicts
		...
		do
			if attached project.targets as l_project_targets then
				a_target_cursor := l_project_targets.new_cursor
				from a_target_cursor.finish until a_target_cursor.before loop
					a_target := a_target_cursor.item
					a_target_name := a_target_cursor.key
					a_target_cursor2 := l_project_targets.new_cursor
	...
	merge_in_renamed_targets (a_parent: GEANT_PARENT)
		...
		do
			if attached a_parent.parent_project as l_parent_project and attached project.targets as l_project_targets then
				...
				from a_rename_cursor.finish until a_rename_cursor.before loop
					...
					l_project_targets.search (a_rename_target_name)
					if l_project_targets.found then
							-- There is already a target named `a_rename_target_name'.
						a_target := l_project_targets.found_item
						if STRING_.same_string (a_target.full_name, a_rename_target.full_name) then
							...
							exit_application (1, <<"%NLOAD ERROR:%N", "Project '", project.name,
								...
								"' inherited from project '", l_parent_project.name, "'.%N",
...
--							"  Use a rename, redefine or undefine clause to resolve the name clash.">>)
						...
						l_project_targets.force_last_new (a_rename_target, a_rename_target_name)
					end
			...
			if attached a_parent.parent_project as l_parent_project and attached project.targets as l_project_targets then
				...
				from a_redefine_cursor.finish until a_redefine_cursor.before loop
					...
					project.trace_debug (<<"Project '", project.name, "': merging in redefined target `",
						...
						") from parent '", l_parent_project.name, "%'">>)
					if not l_project_targets.has (a_redefine_target_name) then
						exit_application (1, <<"%NLOAD ERROR:%N", "Project '", project.name,
							"' does not redefine parent target `", a_redefine_target_name,
							"' as declared.">>)
					end
					a_target := l_project_targets.item (a_redefine_target_name)
			...
			if attached a_parent.parent_project as l_parent_project and attached project.targets as l_project_targets then
				...
				from a_unchanged_cursor.start until a_unchanged_cursor.after loop
					...
					project.trace_debug (<<"Project '", project.name, "': merging in unchanged parent target `",
						...
						") from parent '", l_parent_project.name, "%'">>)
					l_project_targets.search (a_unchanged_target_name)
					if l_project_targets.found then
						...
						if STRING_.same_string (a_target.full_name, a_unchanged_target.full_name) then
							...
							a_unchanged_target.full_name, "') is equal to the existing target.">>)
								...
								-- The targets are different, causing a name clash:
							exit_application (1, <<"%NLOAD ERROR:%N", "Project '", project.name,
								"' contains target%N    `", l_project_targets.item (a_unchanged_target_name).full_name,
								...
								"'%N  inherited from project '", l_parent_project.name, "'.%N",
-- TODO: support undefine first:
--							"  Use a rename, redefine or undefine clause to resolve the name clash.">>)
								"  Use a rename or redefine clause to resolve the name clash.">>)
						end
					end
					l_project_targets.force_last_new (a_unchanged_target, a_unchanged_target_name)
...
invariant
	...
	no_void_parent: not parents.has_void
