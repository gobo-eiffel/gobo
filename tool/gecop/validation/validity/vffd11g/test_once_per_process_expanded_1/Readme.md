...
In this test, there is a once-per-process function `f` in class `AA`. Its result type is an expanded type, but it contains an attribute `name` whose type is reference but not separate. This violates `VFFD-11G`.
