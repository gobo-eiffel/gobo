...
In this test, we have a call `b.set_value (6)` where `b` is a once-per-thread function whose type is expanded. The call to `set_value` sets the value of `value` in the object attached to the latest applicable result of the call to `b`, not in a clone ot this object. This test satisfies `MUGC-2`.
