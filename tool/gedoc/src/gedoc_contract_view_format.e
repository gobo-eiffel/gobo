note
	...
	"[
		Gobo Eiffel Documentation Contract-view Format: do not print feature implementations,
		only exported features with their assertions.
	]"

	copyright: "Copyright (c) 2026, Eric Bezault and others"
	license: "MIT License"

class GEDOC_CONTRACT_VIEW_FORMAT
...
create {GEDOC_CONTRACT_VIEW_FORMAT}
			...
			if not {PLATFORM}.is_thread_capable or else a_class.processing_mutex.try_lock then
				if not a_class.is_marked then
					...
					elseif not attached class_output_directory (a_class) as l_directory then
						...
						l_filename := filename (l_directory, concat (class_lower_name (a_class), contract_view_extension))
						if not is_file_overwritable (l_filename) then
							...
							if l_file.is_open_write then
								l_viewer := contract_viewer
								l_viewer.reset
								l_viewer.set_file (l_file)
								a_class.process (l_viewer)
								l_viewer.set_null_file
								l_file.close
							else
...
feature {NONE} -- Implementation
	...
	add_format (a_system_processor: ET_SYSTEM_PROCESSOR; a_formats: DS_ARRAYED_LIST [like Current])
		...
		do
			...
			a_system_processor.set_custom_processor (agent l_format.contract_view_class)
	...
	contract_viewer: ET_AST_CONTRACT_VIEWER
			-- Contract-viewer

	contract_view_extension: STRING = ".ecv"
			-- Contract-view file extension (with the leading '.')

invariant

	contract_viewer_not_void: contract_viewer /= Void
