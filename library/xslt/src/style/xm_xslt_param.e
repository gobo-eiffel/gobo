...
feature -- Access
	...
	allows_required: BOOLEAN
		...
		do
			Result := not attached {XM_XSLT_FUNCTION} parent_node
		end
...
feature -- Element change
	...
	validate
		...
		do
			is_local := attached {XM_XSLT_TEMPLATE} parent or else attached {XM_XSLT_FUNCTION} parent
			if not is_local and then not is_global_variable then
				...
				if not is_global_variable then
					...
					loop
						...
						if attached {XM_XSLT_PARAM} a_node as a_parameter then
							...
							if attached {XM_XSLT_STYLE_ELEMENT} a_node then
	...
	compile (a_executable: XM_XSLT_EXECUTABLE)
		...
		local
			...
			l_conversion: detachable XM_XPATH_EXPRESSION
		...
		do
			...
			if attached {XM_XSLT_FUNCTION} parent as l_function then
			...
			elseif not is_redundant_variable then
				...
				if attached as_type as l_as_type then
					...
					l_type_checker.static_type_check (static_context, l_supplied_parameter_reference, l_as_type, False, l_role)
				...
				if attached error_value as l_error_value then
					...
					if is_global_variable then
						...
						if attached select_expression as l_select_expression and then l_select_expression.is_computed_expression then l_select_expression.as_computed_expression.set_parent (l_global_param) end
