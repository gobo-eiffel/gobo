...
feature -- Test
	...
	test_infoset
		...
		local
			...
			l_second_last_output: detachable STRING
		do
			...
			l_transformer_factory.create_new_transformer (l_uri_source, dummy_uri)
			l_transformer := l_transformer_factory.created_transformer
			assert ("second_transformer", l_transformer /= Void)
			check asserted_above: l_transformer /= Void then end
			create l_second_uri_source.make (test1_xml_uri.full_reference)
			...
			l_second_last_output := l_second_output.last_output
			assert ("second_set_output_to_string", l_second_last_output /= Void)
			check asserted_above: l_second_last_output /= Void then end
			assert ("Files are canonically the same", STRING_.same_string (l_last_output, l_second_last_output))
		end
