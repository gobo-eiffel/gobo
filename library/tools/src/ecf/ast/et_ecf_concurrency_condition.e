...
feature -- Status report
	...
	is_enabled (a_state: ET_ECF_STATE): BOOLEAN
		...
		do
			l_state_value := a_state.target.capabilities.use_value ({ET_ECF_CAPABILITY_NAMES}.concurrency_capability_name)
			if l_state_value = Void then
				l_state_value := a_state.target.capabilities.support_value ({ET_ECF_CAPABILITY_NAMES}.concurrency_capability_name)
			end
			if l_state_value = Void then
				...
				l_state_value := l_default_capabilities.use_value ({ET_ECF_CAPABILITY_NAMES}.concurrency_capability_name)
				if l_state_value = Void then
					l_state_value := l_default_capabilities.support_value ({ET_ECF_CAPABILITY_NAMES}.concurrency_capability_name)
				end
			end
			if l_state_value /= Void then
