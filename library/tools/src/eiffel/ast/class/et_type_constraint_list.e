	...
	has_expanded_type (a_context: ET_TYPE_CONTEXT): BOOLEAN
		...
		do
			...
			until
				j < 0
			loop
				if storage.item (j).type_constraint.is_type_expanded (a_context) then
					Result := True
					j := 0 -- Jump out of the loop.
				end
				j := j - 1
	...
	has_attached_type (a_context: ET_TYPE_CONTEXT): BOOLEAN
		...
		do
			...
			until
				j < 0
			loop
				if storage.item (j).type_constraint.is_type_attached (a_context) then
					Result := True
					j := 0 -- Jump out of the loop.
				end
				j := j - 1
	...
	has_non_separate_reference_attributes (a_context: ET_TYPE_CONTEXT): BOOLEAN
		...
		do
			...
			until
				j < 0
			loop
				if storage.item (j).type_constraint.has_non_separate_reference_attributes (a_context) then
					Result := True
					j := 0 -- Jump out of the loop.
				end
				j := j - 1
	...
	has_nested_non_separate_reference_attributes (a_context: ET_TYPE_CONTEXT): BOOLEAN
		...
		do
			...
			until
				j < 0
			loop
				if storage.item (j).type_constraint.has_nested_non_separate_reference_attributes (a_context) then
					Result := True
					j := 0 -- Jump out of the loop.
				end
				j := j - 1
