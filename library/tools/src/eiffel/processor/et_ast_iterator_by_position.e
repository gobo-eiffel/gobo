...
feature {ET_AST_NODE} -- Processing
	...
	process_attachment_mark_separate_keyword (a_keywords: ET_ATTACHMENT_MARK_SEPARATE_KEYWORD)
			-- Process `a_keywords'.
		do
			if a_keywords.contains_position (current_position) then
	...
	process_client (a_client: ET_CLIENT)
			-- Process `a_client'.
		do
			if a_client.contains_position (current_position) then
				precursor (a_client)
			end
		end
	...
	process_client_comma (a_client_comma: ET_CLIENT_COMMA)
		...
		do
			if a_client_comma.contains_position (current_position) then
	...
	process_convert_builtin_expression (a_convert_expression: ET_CONVERT_BUILTIN_EXPRESSION)
		...
		do
			if a_convert_expression.contains_position (current_position) then
	...
	process_convert_feature_comma (a_convert_feature: ET_CONVERT_FEATURE_COMMA)
		...
		do
			if a_convert_feature.contains_position (current_position) then
	...
	process_identifier_colon (an_identifier: ET_IDENTIFIER_COLON)
		...
		do
			if an_identifier.contains_position (current_position) then
	...
	process_inline_separate_argument_comma (a_argument_comma: ET_INLINE_SEPARATE_ARGUMENT_COMMA)
		...
		do
			if a_argument_comma.contains_position (current_position) then
	...
	process_note_term_comma (a_note_term: ET_NOTE_TERM_COMMA)
			-- Process `a_note_term'.
		do
			if a_note_term.contains_position (current_position) then
	...
	process_type_constraint_comma (a_type_constraint_comma: ET_TYPE_CONSTRAINT_COMMA)
		...
		do
			if a_type_constraint_comma.contains_position (current_position) then
