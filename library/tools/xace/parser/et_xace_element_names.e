...
feature -- Constants
	...
	uc_system: UC_STRING is
		...
		once
			...
			uc_system_not_empty: not Result.empty
		end

	uc_root: UC_STRING is
			-- "root" element name
		once
			!! Result.make_from_string ("root")
		ensure
			uc_root_not_void: Result /= Void
			uc_root_not_empty: not Result.empty
		end

	uc_class: UC_STRING is
			-- "class" attribute name
		once
			!! Result.make_from_string ("class")
		ensure
			uc_class_not_void: Result /= Void
			uc_class_not_empty: not Result.empty
		end

	uc_creation: UC_STRING is
			-- "creation" attribute name
		once
			!! Result.make_from_string ("creation")
		ensure
			uc_creation_not_void: Result /= Void
			uc_creation_not_empty: not Result.empty
		end
	...
	uc_cluster: UC_STRING is
		...
		once
			...
			uc_cluster_not_empty: not Result.empty
		end
	...
	uc_name: UC_STRING is
		...
		once
			...
			uc_name_not_empty: not Result.empty
		end

	uc_abstract: UC_STRING is
			-- "abstract" attribute name
		once
			!! Result.make_from_string ("abstract")
		ensure
			uc_abstract_not_void: Result /= Void
			uc_abstract_not_empty: not Result.empty
		end

	uc_mount: UC_STRING is
			-- "mount" element name
		once
			!! Result.make_from_string ("mount")
		ensure
			uc_mount_not_void: Result /= Void
			uc_mount_not_empty: not Result.empty
		end

	uc_location: UC_STRING is
			-- "location" attribute name
		once
			!! Result.make_from_string ("location")
		ensure
			uc_location_not_void: Result /= Void
			uc_location_not_empty: not Result.empty
		end
	...
	uc_option: UC_STRING is
			-- "option" element name
		once
			!! Result.make_from_string ("option")
		ensure
			uc_option_not_void: Result /= Void
			uc_option_not_empty: not Result.empty
		end

	uc_external: UC_STRING is
			-- "external" element name
		once
			!! Result.make_from_string ("external")
		ensure
	...
	uc_enable: UC_STRING is
		...
		once
			!! Result.make_from_string ("enable")
		ensure
			uc_enable_not_void: Result /= Void
			uc_enable_not_empty: not Result.empty
		end

	uc_require: UC_STRING is
			-- "require" element name
		once
			!! Result.make_from_string ("require")
		ensure
			uc_require_not_void: Result /= Void
			uc_require_not_empty: not Result.empty
		end

	uc_ensure: UC_STRING is
			-- "ensure" element name
		once
			!! Result.make_from_string ("ensure")
		ensure
			uc_ensure_not_void: Result /= Void
			uc_ensure_not_empty: not Result.empty
		end

	uc_invariant: UC_STRING is
			-- "invariant" element name
		once
			!! Result.make_from_string ("invariant")
		ensure
			uc_invariant_not_void: Result /= Void
			uc_invariant_not_empty: not Result.empty
		end

	uc_loop: UC_STRING is
			-- "loop" element name
		once
			!! Result.make_from_string ("loop")
		ensure
			uc_loop_not_void: Result /= Void
			uc_loop_not_empty: not Result.empty
		end

	uc_check: UC_STRING is
			-- "check" element name
		once
			!! Result.make_from_string ("check")
		ensure
			uc_check_not_void: Result /= Void
			uc_check_not_empty: not Result.empty
		end

	uc_debug: UC_STRING is
			-- "debug" element name
		once
			!! Result.make_from_string ("debug")
		ensure
			uc_debug_not_void: Result /= Void
			uc_debug_not_empty: not Result.empty
		end

	uc_optimize: UC_STRING is
			-- "optimize" element name
		once
			!! Result.make_from_string ("optimize")
		ensure
			uc_optimize_not_void: Result /= Void
			uc_optimize_not_empty: not Result.empty
		end

	uc_link_dir: UC_STRING is
			-- "link_dir" element name
		once
			!! Result.make_from_string ("link_dir")
		ensure
			uc_link_dir_not_void: Result /= Void
			uc_link_dir_not_empty: not Result.empty
		end

	uc_include_dir: UC_STRING is
			-- "include_dir" element name
		once
			!! Result.make_from_string ("include_dir")
		ensure
			uc_include_dir_not_void: Result /= Void
			uc_include_dir_not_empty: not Result.empty
		end

	uc_export: UC_STRING is
			-- "export" element name
		once
			!! Result.make_from_string ("export")
		ensure
			uc_export_not_void: Result /= Void
			uc_export_not_empty: not Result.empty
		end

	uc_alias: UC_STRING is
			-- "alias" attribute name
		once
			!! Result.make_from_string ("alias")
		ensure
			uc_alias_not_void: Result /= Void
			uc_alias_not_empty: not Result.empty
		end

	uc_feature: UC_STRING is
			-- "feature" element name
		once
			!! Result.make_from_string ("feature")
		ensure
			uc_feature_not_void: Result /= Void
			uc_feature_not_empty: not Result.empty
		end

	uc_link_library: UC_STRING is
			-- "link_library" element name
		once
			!! Result.make_from_string ("link_library")
		ensure
			uc_link_library_not_void: Result /= Void
			uc_link_library_not_empty: not Result.empty
		end
