...
inherit
	...
	ET_AST_LIST [ET_FORMAL_PARAMETER_ITEM]
		redefine
...
feature -- Validity
	...
	check_validity (a_class: ET_CLASS): BOOLEAN is
		...
		local
			i, j, nb: INTEGER
			a_formal: like formal_parameter
			a_name: ET_IDENTIFIER
			a_universe: ET_UNIVERSE
			other_class: ET_CLASS
			a_constraint: ET_TYPE
			a_formal_constraint: ET_FORMAL_PARAMETER_TYPE
			nb_constraints: INTEGER
			an_index: INTEGER
			a_sorter: DS_TOPOLOGICAL_SORTER [ET_FORMAL_PARAMETER]
		do
			Result := True
			a_universe := a_class.universe
			a_sorter := a_universe.formal_parameter_sorter
			a_sorter.wipe_out
			nb := count
			from i := 1 until i > nb loop
				...
				if a_universe.has_class (a_name) then
					...
					from j := 1 until j >= i loop
						if formal_parameter (j).name.same_identifier (a_name) then
							a_class.error_handler.report_vcfg2_error (a_class, formal_parameter (j), a_formal)
							Result := False
					...
					if a_constraint /= Void then
						...
						if a_formal_constraint /= Void then
							...
							elseif an_index < i then
								a_sorter.force_relation (formal_parameter (an_index), a_formal)
...
feature -- Type processing
	...
	resolve_named_types (a_class: ET_CLASS; ast_factory: ET_AST_FACTORY) is
		...
		do
			nb := count
			from i := 1 until i > nb loop
				a_constrained_formal ?= formal_parameter (i)
...
feature -- System
	...
	add_to_system is
		...
		do
			nb := count
			from i := 1 until i > nb loop
				formal_parameter (i).add_to_system
				i := i + 1
			end
		end
