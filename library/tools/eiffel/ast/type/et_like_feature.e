			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.is_cat_type (a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.is_cat_type (a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.is_actual_cat_type (i, a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.is_actual_cat_type (i, a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.is_cat_parameter (a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.is_cat_parameter (a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.is_actual_cat_parameter (i, a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.is_actual_cat_parameter (i, a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.has_forget_feature (a_feature, a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.has_forget_feature (a_feature, a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
			...
			elseif is_like_argument then
				...
				if seeded_feature /= Void then
					...
					if args = Void or else an_index > args.count then
						...
						Result := args.item (an_index).type.has_actual_forget_feature (i, a_feature, a_context, a_universe)
					end
				else
						-- Internal error: an inconsistency has been
						-- introduced in the AST since we relsolved
				...
				if seeded_feature /= Void then
					a_query_type := seeded_feature.type
					if a_query_type /= Void then
						Result := a_query_type.has_actual_forget_feature (i, a_feature, a_context, a_universe)
					else
							-- Internal error: an inconsistency has been
							-- introduced in the AST since we relsolved
							-- current anchored type.
