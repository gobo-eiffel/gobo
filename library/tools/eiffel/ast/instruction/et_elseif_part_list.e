...
feature {NONE} -- Initialization
	...
	make (an_elseif_part: like item) is
			-- Create a new elseif part list with `an_elseif_part' as first item.
		require
			an_elseif_part_not_void: an_elseif_part /= Void
		do
			make_ast_list_with_capacity (1)
			put_first (an_elseif_part)
		ensure
			elseif_part_set: item (1) = an_elseif_part
			capacity_set: capacity = 1
		end

	make_with_capacity (an_elseif_part: like item; nb: INTEGER) is
			-- Create a new elseif part list with capacity `nb'
			-- and `an_elseif_part' as first item.
		require
			nb_positive: nb >= 1
			an_elseif_part_not_void: an_elseif_part /= Void
		do
