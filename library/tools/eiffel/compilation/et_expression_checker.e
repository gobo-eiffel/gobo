		...
		do
			...
			if a_seed = 0 then
				...
				if a_class_impl.has_interface_error then
					...
					if a_feature /= Void then
						...
						error_handler.report_vpca1a_error (a_class_impl, a_name)
					end
				end
			end
			if not has_fatal_error and a_seed /= 0 then
				...
				if a_feature /= Void then
					...
					check_agent_arguments_validity (an_actuals, a_name, a_feature, l_formal_context, Void, an_open_operands)
					...
					if a_class.has_interface_error then
						...
						if a_feature /= Void then
							...
							set_fatal_error
								-- ISE Eiffel 5.4 reports this error as a VEEN,
								-- but it is in fact a VPCA-1 (ETL3-4.82-00-00 p.581).
							error_handler.report_vpca1b_error (a_class_impl, a_name, a_class)
						end
					end
				end
			end
...
-- a local variable, a formal argument or the name of an attribute.
				...
				if a_feature /= Void then
					...
					check_agent_arguments_validity (an_actuals, a_name, a_feature, l_target_context, a_class, an_open_operands)
				...
				if not has_fatal_error and a_seed /= 0 then
					if a_feature = Void then
						...
						if not has_fatal_error then
							...
							l_target_context.force_first (a_target_type)
							a_target_type := tokens.like_current
							a_class := l_target_context.base_class (universe)
							a_class.process (universe.interface_checker)
							if a_class.has_interface_error then
...
--								end
	...
	check_agent_arguments_validity (an_actuals: ET_AGENT_ACTUAL_ARGUMENT_LIST; a_name: ET_FEATURE_NAME;
		a_feature: ET_FEATURE; a_context: ET_NESTED_TYPE_CONTEXT; a_class: ET_CLASS;
		an_open_operands: ET_ACTUAL_PARAMETER_LIST) is
			...
			-- not Void, is where to store the type of open operands.
		require
			a_name_not_void: a_name /= Void
			a_feature_not_void: a_feature /= Void
					...
					if a_formals /= Void and then not a_formals.is_empty then
						...
						if a_class /= Void then
							if current_class = a_class_impl then
								...
								error_handler.report_vpca3b_error (current_class, a_class_impl, a_name, a_feature, a_class)
							end
						else
							if current_class = a_class_impl then
								error_handler.report_vpca3c_error (current_class, a_name, a_feature)
							else
								error_handler.report_vpca3d_error (current_class, a_class_impl, a_name, a_feature)
							end
						end
					end
...
-- which works only in a limited number of cases).
						...
						if a_like /= Void and then a_like.is_like_argument then
							if l_target_context /= a_context then
								...
								check_expression_validity (an_expression.right, a_context, l_target_context, current_feature, current_class)
