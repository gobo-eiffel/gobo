...
feature -- Access
	...
	variable_value (a_variable: STRING): STRING is
		...
		do
			Result := environment_impl.get_environment_variable (STRING_.as_string (a_variable))
...
feature -- Setting
	...
	set_variable_value (a_variable, a_value: STRING) is
		...
		do
			environment_impl.set_environment_variable (STRING_.as_string (a_variable), STRING_.as_string (a_value))
