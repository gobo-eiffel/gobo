...
feature -- Pathname
	...
	uri_to_pathname (a_uri: UT_URI): KL_PATHNAME is
		...
		do
			...
			if not a_cursor.after then
				...
				if False then-- a_uri.has_absolute_path and then file_system.is_drive (a_possible_drive) then
			...
			debug ("file_uri")
				io.put_string ("u2p "+a_uri.full_reference+" "+file_system.pathname_to_string (Result))
				io.put_new_line
			end
		ensure
			result_not_void: Result /= Void
	...
	pathname_to_uri (a_pathname: KI_PATHNAME): UT_URI is
		...
		do
			...
			debug ("file_uri")
				io.put_string ("p2u "+file_system.pathname_to_string (a_pathname)+" "+Result.full_reference)
				io.put_new_line
			end
		end
