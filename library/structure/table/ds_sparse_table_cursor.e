...
feature -- Access
	...
	key: K is
		...
		require
			not_off: not off
		do
			Result := container.keys_item (position)
		end
...
feature -- Status report
	...
	is_first: BOOLEAN is
		...
		do
			a_table := container
			if not a_table.is_empty then
				from
					i := 1
				until
					a_table.clashes_item (i) > Free_watermark
				loop
					i := i + 1
				end
				Result := (position = i)
...
feature -- Cursor movement
	...
	start is
		...
		do
			a_table := container
			if a_table.is_empty then
				position := After_position
			else
				was_off := off
				from
					i := 1
					nb := a_table.last_position
				until
					i > nb or else
					a_table.clashes_item (i) > Free_watermark
				loop
