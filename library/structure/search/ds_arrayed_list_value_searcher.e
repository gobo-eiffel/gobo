indexing
	...
	description:
		...
		"Arrayed list searchers using value comparison criterion"
...
class DS_ARRAYED_LIST_VALUE_SEARCHER [G]
	...
	search_forth (a_cursor: like cursor; v: G) is
		...
		do
			from
				...
				nb := list.count
			until
				i > nb or else equal (list.item (i), v)
			loop
				i := i + 1
			end
			a_cursor.go_to (i)
	...
	search_back (a_cursor: like cursor; v: G) is
		...
		do
			from
				...
				list := a_cursor.container
			until
				i < 1 or else equal (list.item (i), v)
			loop
				i := i - 1
			end
			a_cursor.go_to (i)
