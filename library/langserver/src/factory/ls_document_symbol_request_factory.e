...
feature -- Access
	...
	new_message (a_object: LS_OBJECT; a_manager: LS_MESSAGE_MANAGER): LS_MESSAGE
		...
		do
			if not attached request_id_in_object (a_object, {LS_DOCUMENT_SYMBOL_REQUEST}.id_name, False) as l_id then
				l_error_message := if attached last_error as l_last_error then l_last_error else {LS_DOCUMENT_SYMBOL_REQUEST}.id_name + ": missing field" end
			...
			elseif not attached {LS_OBJECT} a_object.value ({LS_DOCUMENT_SYMBOL_REQUEST}.params_name) as l_params then
				l_error_message := {LS_DOCUMENT_SYMBOL_REQUEST}.params_name + ": missing field"
				create l_error.make ({LS_ERROR_CODES}.invalid_params, l_error_message)
				create l_response.make_error (l_id, l_error)
				create {LS_HANDLED_MESSAGE} Result.make (l_response)
			elseif not attached text_document_identifier_in_object (l_params, {LS_DOCUMENT_SYMBOL_REQUEST}.text_document_name, False) as l_text_document then
				l_error_message := if attached last_error as l_last_error then l_last_error else {LS_DOCUMENT_SYMBOL_REQUEST}.text_document_name + ": missing field" end
				l_error_message := {LS_DOCUMENT_SYMBOL_REQUEST}.params_name + "." + l_error_message
				create l_error.make ({LS_ERROR_CODES}.invalid_params, l_error_message)
				create l_response.make_error (l_id, l_error)
				create {LS_HANDLED_MESSAGE} Result.make (l_response)
			else
				l_work_done_token := progress_token_in_object (l_params, {LS_DOCUMENT_SYMBOL_REQUEST}.work_done_token_name, True)
				if attached last_error as l_last_error then
					l_error_message := {LS_DOCUMENT_SYMBOL_REQUEST}.params_name + "." + l_last_error
					create l_error.make ({LS_ERROR_CODES}.invalid_params, l_error_message)
					create l_response.make_error (l_id, l_error)
					create {LS_HANDLED_MESSAGE} Result.make (l_response)
				end
				if Result = Void then
					l_partial_result_token := progress_token_in_object (l_params, {LS_DOCUMENT_SYMBOL_REQUEST}.partial_result_token_name, True)
					if attached last_error as l_last_error then
						l_error_message := {LS_DOCUMENT_SYMBOL_REQUEST}.params_name + "." + l_last_error
						create l_error.make ({LS_ERROR_CODES}.invalid_params, l_error_message)
						create l_response.make_error (l_id, l_error)
				...
				if Result = Void then
					create {LS_DOCUMENT_SYMBOL_REQUEST} Result.make (l_id, l_text_document, l_work_done_token, l_partial_result_token)
				end
			end
		end

	new_server_registration_options (a_registration: LS_REGISTRATION; a_manager: LS_MESSAGE_MANAGER): detachable LS_DOCUMENT_SYMBOL_REGISTRATION_OPTIONS
		...
		do
			...
			elseif attached document_symbol_registration_options_from_object (l_object, {LS_REGISTRATION}.register_options_name) as l_document_symbol_registration_options then
