...
class LS_DOCUMENT_HIGHLIGHT_REQUEST

inherit

	LS_REQUEST

	LS_DOCUMENT_HIGHLIGHT_PARAMS
...
feature {NONE} -- Initialization
	...
	make (a_id: like id; a_text_document: like text_document; a_position: like position; a_work_done_token: like work_done_token; a_partial_result_token: like partial_result_token)
			-- Create a new 'textDocument/documentHighlight' request.
		require
...
feature -- Access
	...
	handler (a_manager: LS_MESSAGE_MANAGER): LS_DOCUMENT_HIGHLIGHT_REQUEST_HANDLER
		...
		do
			Result := a_manager.document_highlight_request_handler
		ensure then
			instance_free: class
		end
...
feature -- Processing
	...
	process (a_processor: LS_PROCESSOR)
		...
		do
			a_processor.process_document_highlight_request (Current)
		end
