note
	...
	description:
		...
		"LSP handlers for 'textDocument/implementation' requests"

	library: "Gobo Eiffel Language Server Protocol Library"
	copyright: "Copyright (c) 2025, Eric Bezault and others"
	license: "MIT License"

class LS_IMPLEMENTATION_REQUEST_HANDLER
...
feature -- Access
	...
	client_capabilities: detachable LS_IMPLEMENTATION_CAPABILITIES
			-- Client capabilities

	server_options: detachable LS_OPTIONAL_IMPLEMENTATION_OPTIONS
			-- Server options
...
feature -- Setting
	...
	build_server_options
		...
		do
			...
			elseif not generating_type.same_type ({detachable LS_IMPLEMENTATION_REQUEST_HANDLER}) then
...
feature {NONE} -- Implementation
	...
	request: LS_IMPLEMENTATION_REQUEST
			-- Type of request to be handled by current handler
		do
			check False then end
		end

	registration_options: LS_IMPLEMENTATION_REGISTRATION_OPTIONS
			-- Type of the dynamic registration options
		do
			check False then end
		end

	response_result: LS_OPTIONAL_IMPLEMENTATION_RESULT
