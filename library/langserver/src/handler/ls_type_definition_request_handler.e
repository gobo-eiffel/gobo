note
	...
	description:
		...
		"LSP handlers for 'textDocument/typeDefinition' requests"

	library: "Gobo Eiffel Language Server Protocol Library"
	copyright: "Copyright (c) 2025, Eric Bezault and others"
	license: "MIT License"

class LS_TYPE_DEFINITION_REQUEST_HANDLER
...
feature -- Access
	...
	client_capabilities: detachable LS_TYPE_DEFINITION_CAPABILITIES
			-- Client capabilities

	server_options: detachable LS_OPTIONAL_TYPE_DEFINITION_OPTIONS
			-- Server options
...
feature -- Setting
	...
	build_server_options
		...
		do
			...
			elseif not generating_type.same_type ({detachable LS_TYPE_DEFINITION_REQUEST_HANDLER}) then
...
feature {NONE} -- Implementation
	...
	request: LS_TYPE_DEFINITION_REQUEST
			-- Type of request to be handled by current handler
		do
			check False then end
		end

	registration_options: LS_TYPE_DEFINITION_REGISTRATION_OPTIONS
			-- Type of the dynamic registration options
		do
			check False then end
		end

	response_result: LS_OPTIONAL_TYPE_DEFINITION_RESULT
