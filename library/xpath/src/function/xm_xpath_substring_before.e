				...
				if attached a_result.item as a_result_item_1 and then a_result_item_1.is_error then
					...
					if attached a_result.item as a_result_item_2 and then a_result_item_2.is_error then
						...
						elseif l_s1.count = 0 then
							...
							a_result.put (create {XM_XPATH_STRING_VALUE}.make (substring_before (l_s1, l_s2, l_collator)))
						end
					end
				end
			end
...
feature {NONE} -- Implementation
	...
	substring_before (s1, s2: STRING; a_collator: ST_COLLATOR): STRING
			-- Substring of s1 before first occurrence of s2 according to the rules of `a_collator'?
		require
