	...
	evaluate_item (a_result: DS_CELL [detachable XM_XPATH_ITEM]; a_context: XM_XPATH_CONTEXT)
		...
		do
			...
			if attached a_result.item as a_result_item and then a_result_item.is_error then
				-- nothing to do
			elseif not attached a_result.item as a_result_item or else not a_result_item.is_atomic_value then
				a_result.put (create {XM_XPATH_STRING_VALUE}.make (""))
			else
				a_result.put (create {XM_XPATH_STRING_VALUE}.make (upper_utf8_string (a_result_item.as_atomic_value.string_value)))
			end
		end
