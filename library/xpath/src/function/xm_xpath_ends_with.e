...
feature {NONE} -- Initialization
	...
	make
		...
		do
			...
			fingerprint := Ends_with_function_type_code
			...
			if l_collator = Void then
				...
				if attached a_result.item as a_result_item_1 and then a_result_item_1.is_error then
					...
					if attached a_result.item as a_result_item_2 and then a_result_item_2.is_error then
						-- nothing more to do
					elseif not attached a_result.item as a_result_item_2 then
						...
						elseif l_s1.count = 0 then
							...
							a_result.put (create {XM_XPATH_BOOLEAN_VALUE}.make (ends_with (l_s1, l_s2, l_collator)))
						end
					end
				end
			end
...
feature {NONE} -- Implementation
	...
	ends_with (s1, s2: STRING; a_collator: ST_COLLATOR): BOOLEAN
