		...
		do
			...
			if attached a_result.item as a_result_item and then not a_result_item.is_error then
				...
				if l_date_time_value.zoned then
					check zoned: attached l_date_time_value.zoned_date_time as l_zoned_date_time then
						create l_date_duration.make (0, 0, 0)
						create l_dt_duration.make_from_date_time_duration (l_date_duration, l_zoned_date_time.time_zone.fixed_offset)
