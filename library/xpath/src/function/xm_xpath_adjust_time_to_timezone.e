note
	...
	description:
		...
		"Objects that implement the XPath adjust-time-to-timezone() function"
...
class XM_XPATH_ADJUST_TIME_TO_TIMEZONE
...
feature {NONE} -- Initialization
	...
	make
		...
		do
			...
			fingerprint := Adjust_time_to_timezone_function_type_code
	...
	evaluate_item (a_result: DS_CELL [detachable XM_XPATH_ITEM]; a_context: XM_XPATH_CONTEXT)
		...
		do
			...
			if attached a_result.item as a_result_item and then not a_result_item.is_error then
				l_time_value := a_result_item.as_atomic_value.as_time_value
				if arguments.count = 1 then
					...
					if not attached a_result.item as a_result_item_2 then
						a_result.put (l_time_value.as_zoneless)
						l_finished := True
					elseif not a_result_item_2.is_error then
				...
				if not l_finished then
					check attached l_duration_value then
						a_result.put (l_time_value.to_another_time_zone (l_duration_value))
					end
				end
			end
		end
