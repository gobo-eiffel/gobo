indexing
	...
	description: "Class for parsing XML documents using Expat."
...
feature {NONE} -- (low level) frozen callbacks (called from eXML clib)
	...
	frozen on_entity_declaration_procedure (entity_name_ptr: POINTER;
		...
		do
			...
			base :=
							new_uc_string_from_c_utf8_zero_terminated_string_safe (base_ptr)
				system_id :=
								new_uc_string_from_c_utf8_zero_terminated_string_safe (system_id_ptr)
				public_id :=
