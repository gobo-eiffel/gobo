	...
	process_leaving_tail (a_context: XM_XSLT_EVALUATION_CONTEXT) is
		...
		do
			...
			if a_current_template = Void then
				create an_error.make_from_string ("Current template rule is null whilst evaluating xsl:next-match.", "", "XT0560", Dynamic_error)
				...
				if a_current_iterator = Void or else a_current_iterator.off then
					create an_error.make_from_string ("Context item is not set whilst evaluating xsl:next-match.", "", "XT0565", Dynamic_error)
					a_transformer.report_fatal_error (an_error, Void)
				else
					a_context_node ?= a_current_iterator.item
					if a_context_node = Void then
						create an_error.make_from_string ("Context item is not a node whilst evaluating xsl:next-match.", "", "XT0565", Dynamic_error)
						a_transformer.report_fatal_error (an_error, Void)
					else
