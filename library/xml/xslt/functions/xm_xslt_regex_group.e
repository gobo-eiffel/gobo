	...
	evaluate_item (a_context: XM_XPATH_CONTEXT) is
		...
		do
			...
			if arguments.item (1).last_evaluated_item.is_error then
				...
				check
					integer: arguments.item (1).last_evaluated_item.is_integer_value
					-- Static typing
				end
				an_evaluation_context ?= a_context
				a_regex_iterator := an_evaluation_context.current_regexp_iterator
				if a_regex_iterator /= Void then
					an_integer_value := arguments.item (1).last_evaluated_item.as_integer_value
					if an_integer_value.is_platform_integer then
						create {XM_XPATH_STRING_VALUE} last_evaluated_item.make (a_regex_iterator.regex_group (arguments.item (1).as_integer_value.as_integer))
					else
