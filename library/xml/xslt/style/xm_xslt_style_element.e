	...
	check_top_level is
		...
		do
			if not is_top_level then
				create an_error.make_from_string (STRING_.concat (node_name, " must only be used at top level of stylesheet"), "", "XT0010", Static_error)
				report_compile_error (an_error)
			end
		end
	...
	check_empty_with_attribute (an_attribute_name: STRING) is
		...
		do
			if has_child_nodes then
				...
				create an_error.make_from_string (a_message, "", "XT0260", Static_error)
				report_compile_error (an_error)
			end
		end
	...
	fallback_processing (an_executable: XM_XSLT_EXECUTABLE; a_style_element: XM_XSLT_STYLE_ELEMENT; an_instruction_list: DS_LINKED_LIST [XM_XSLT_INSTRUCTION]) is
		...
		do
			...
			if not found_fallback then
				create a_deferred_error.make (an_executable, a_style_element.validation_error, a_style_element.node_name)
