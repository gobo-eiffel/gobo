indexing
	...
	description:
		...
		"Objects that implement gexslt:document"
...
class XM_XSLT_GEXSLT_DOCUMENT

inherit

	XM_XSLT_STYLE_ELEMENT
		redefine
			validate, is_gexslt_document
		end
...
feature -- Element change
	...
	validate is
		...
		do
			...
			if an_isolation_levels = Void then
				create an_error.make_from_string ("Gexslt:document must be immediate child of gexslt:isolation-levels", Xpath_errors_uri, "XTSE0010", Static_error)
	...
	compile (an_executable: XM_XSLT_EXECUTABLE) is
		...
		local
			an_error: XM_XPATH_ERROR_VALUE
		do
			if an_executable.has_document_isolation_level (href) then
				if an_executable.document_isolation_level (href) /= isolation_level then
					create an_error.make_from_string ("Conflicting definitions for isolation-level attribute of gexslt:document",
																 Gexslt_eiffel_type_uri, "ISOLATION-LEVEL", Static_error)
					report_compile_error (an_error)
				end
			else
				an_executable.set_document_isolation_level (isolation_level, href)
			end
