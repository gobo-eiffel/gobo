	...
	dynamic_error_value (a_context: XM_XPATH_CONTEXT): XM_XPATH_ERROR_VALUE is
		...
		do
			if a_context = Void then
				create Result.make_from_string ("Evaluating '/': the dynamic context is not available", Xpath_errors_uri, "XP0002", Dynamic_error)
			else
				an_item := a_context.context_item
				if an_item = Void then
					create Result.make_from_string ("Evaluating '/': the context item is not set", Xpath_errors_uri, "XP0002", Dynamic_error)
				else
					a_node ?= an_item
					if a_node = Void then
						create Result.make_from_string ("Evaluating '/': the context item is not a node", Xpath_errors_uri, "XP0020", Type_error)
					else
							check
								a_node.document_root = Void
								-- follows from pre-condition
							end
						create Result.make_from_string ("Evaluating '/': the root of the tree containing the context item is not a document node", Xpath_errors_uri, "XP0020", Type_error)
					end
				end
			end
		end
