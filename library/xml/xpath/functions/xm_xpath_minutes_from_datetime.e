indexing
	...
	description:
		...
		"Objects that implement the XPath minutes-from-dateTime() function"
...
class XM_XPATH_MINUTES_FROM_DATETIME
...
feature {NONE} -- Initialization
	...
	make is
		...
		do
			...
			fingerprint := Minutes_from_datetime_function_type_code
	...
	evaluate_item (a_context: XM_XPATH_CONTEXT) is
		...
		do
			...
			if last_evaluated_item /= Void and then not last_evaluated_item.is_error then
				...
				create {XM_XPATH_INTEGER_VALUE} last_evaluated_item.make_from_integer (a_date_time_value.time.minute)
			end
		end
