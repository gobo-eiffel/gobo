indexing
	...
	description:
		...
		"Abstract formatter, raises exceptions calls from parser for%
		%any not recognized type."
...
feature {GF_SCIENTIFIC_FORMAT} -- Type that can be formatted
	...
	anchor: ANY is
			...
			-- Should be initialized, so it can be used in conforms_to checks.
		deferred
		end


feature {GF_ABSTRACT_FORMATTER} -- Exception handling

	class_name: STRING is
			--- Name of class, redefine in descendants
		once
			Result := "G_SCIENTIFIC_FORMATTER"
		ensure
	...
	raise_exception (msg: STRING) is
		...
		do
			...
			s.append_string (msg)
			s.append_character (')')
			the_error_handler.report (class_name, s, 0, Current)
		end
...
feature -- Output
	...
	output: STRING is
			-- Formatted `value';
			-- `value' can be changed in the process if needed.
		deferred
		ensure
...
invariant
	...
	have_class_name: class_name /= Void and then not class_name.is_empty
