...
cd calculator
if [ "$1" = "-finalize" ]; then
	cp $GOBO/example/lexical/calculator/hact.ace Ace.ace
	...
	sed "s/--assertion (all)/assertion (all)/g" $GOBO/example/lexical/calculator/hact.ace > Ace.ace
...
if [ -x calc ]; then
	$GOBO/test/all/common/test_calculator.sh > tmp12.txt 2>&1
	if [ -s tmp12.txt ]; then
		cat tmp12.txt
		echo "Test failed"
	else
