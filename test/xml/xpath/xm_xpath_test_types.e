...
feature -- Test
	...
	test_types is
		do
			...
			assert ("super type", super_type (Unsigned_short_type) = Unsigned_int_type)
			assert ("is sub-type", is_sub_type (Unsigned_short_type, Byte_type) = False)
			assert ("is sub-type e", is_sub_type (Unsigned_short_type, Integer_type) = True)
			assert ("is type name valid", is_type_name_valid ("xs:float") = True)
			assert ("is Qname valid type", is_qname_valid_type (Xml_schema_uri, "float") = True)
			assert ("is Qname valid type 2", is_qname_valid_type (Xml_schema_uri, "flooat") = False)
			assert ("is Qname valid type 3", is_qname_valid_type (Xml_uri, "float") = False)
