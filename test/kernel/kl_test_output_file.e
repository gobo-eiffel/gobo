...
feature -- Test
	...
	test_open_write is
		...
		do
			...
			assert_files_equal ("empty", "$GOBO/test/kernel/data/empty.txt", a_name)
	...
	test_put_new_line is
		...
		do
			...
			if a_file.is_open_write then
				...
				a_file.close
				assert ("is_closed", a_file.is_closed)
				assert_files_equal ("diff", "$GOBO/test/kernel/data/gobo.txt", a_name)
				file_system.delete_file (a_name)
			else
				assert ("is_opened", False)
			end
	...
	test_put_integer is
		...
		do
			...
			if a_file.is_open_write then
				...
				a_file.close
				assert ("is_closed", a_file.is_closed)
				assert_files_equal ("diff", "$GOBO/test/kernel/data/integers.txt", a_name)
				file_system.delete_file (a_name)
			else
				assert ("is_opened", False)
			end
	...
	test_put_boolean is
		...
		do
			...
			if a_file.is_open_write then
				...
				a_file.close
				assert ("is_closed", a_file.is_closed)
				assert_files_equal ("diff", "$GOBO/test/kernel/data/booleans.txt", a_name)
				file_system.delete_file (a_name)
			else
				assert ("is_opened", False)
			end
	...
	test_rename_file1 is
		...
		do
			old_name := "gobo107.tmp"
			new_name := "gobo108.tmp"
	...
	test_rename_file2 is
		...
		local
			...
			old_name, new_name: STRING
		do
			old_name := "gobo109.tmp"
			new_name := "gobo110.tmp"
	...
	test_rename_file3 is
		...
		do
			old_name := "gobo111.tmp"
			new_name := "gobo112.tmp"
	...
	test_rename_file4 is
		...
		do
			old_name := "gobo113.tmp"
			new_name := "gobo114.tmp"
