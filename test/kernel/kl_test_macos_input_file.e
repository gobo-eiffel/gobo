indexing
	...
	description:
		...
		"Test features of class KL_MACOS_INPUT_FILE"
...
deferred class KL_TEST_MACOS_INPUT_FILE
		...
		do
			...
			if out_file.is_open_write then
				...
				if a_file.is_open_read then
					a_file.read_new_line
					assert ("not_eof1", not a_file.end_of_file)
					assert_equal ("new_line", "%R", a_file.last_string)
					a_file.read_character
					assert ("eof1", a_file.end_of_file)
					a_file.close
					assert ("is_closed2", a_file.is_closed)
...
end -- class KL_TEST_MACOS_INPUT_FILE
