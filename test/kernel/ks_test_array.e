...
feature -- Test
	...
	test_subarray is
		...
		do
			...
			assert_equal ("lower_set", 3, l_ai2.lower)
			assert_equal ("upper_set", 5, l_ai2.upper)
			...
			if not eiffel_compiler.is_ise then
				...
				assert_equal ("lower_set2", 6, l_ai2.lower)
				assert_equal ("upper_set2", 5, l_ai2.upper)
					-- Subarray of an empty array.
				create l_ai1.make (4, 3)
				l_ai2 := l_ai1.subarray (4, 3)
				assert_equal ("same_items3", 0, l_ai2.count)
				assert_equal ("lower_set3", 4, l_ai2.lower)
				assert_equal ("upper_set3", 3, l_ai2.upper)
			end
		end
	...
	test_clear_all is
		...
		do
			...
			assert_equal ("ai_lower_set", 4, l_ai.lower)
			assert_equal ("ai_upper_set", 7, l_ai.upper)
			l_ai.clear_all
			assert_iarrays_same ("ai_all_cleared", <<0, 0, 0, 0>>, l_ai)
			assert_equal ("ai_same_lower", 4, l_ai.lower)
			assert_equal ("ai_same_upper", 7, l_ai.upper)
			l_as := <<"1", "2", "3", "4">>
			...
			assert_equal ("as_empty2", 0, l_as.count)
			l_as.clear_all
			assert_equal ("as_still_empty2", 0, l_as.count)
			assert_equal ("as_same_lower2", 3, l_as.lower)
			assert_equal ("as_same_upper2", 2, l_as.upper)
		end
