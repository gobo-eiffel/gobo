indexing
	...
	description:
		...
		"Test features of class KL_WINDOWS_INPUT_FILE"
...
deferred class KL_TEST_WINDOWS_INPUT_FILE
...
feature -- Test
	...
	test_eol is
		...
		do
			a_name := new_filename ("gobo", ".tmp")
			!! a_file.make (a_name)
			assert_equal ("eol", "%R%N", a_file.eol)
		end
	...
	test_change_name1 is
		...
		local
			a_file: KL_WINDOWS_INPUT_FILE
			old_file: KL_WINDOWS_OUTPUT_FILE
	...
	test_change_name3 is
		...
		local
			a_file: KL_WINDOWS_INPUT_FILE
			old_name, new_name: STRING
			new_file: KL_WINDOWS_OUTPUT_FILE
		do
			old_name := new_filename ("gobo", ".tmp")
			new_name := new_filename ("gobo", ".tmp")
	...
	test_change_name4 is
		...
		local
			a_file: KL_WINDOWS_INPUT_FILE
			old_name, new_name: STRING
			out_file: KL_WINDOWS_OUTPUT_FILE
		do
			old_name := new_filename ("gobo", ".tmp")
			new_name := new_filename ("gobo", ".tmp")
	...
	test_windows is
			-- Test that file is a windows file.
		local
			a_file: KL_WINDOWS_INPUT_FILE
		...
		do
			...
			if out_file.is_open_write then
				...
				if a_file.is_open_read then
					a_file.read_new_line
					assert ("not_eof1", not a_file.end_of_file)
					assert_equal ("new_line", "%R%N", a_file.last_string)
					a_file.read_character
					assert ("eof1", a_file.end_of_file)
					a_file.close
					assert ("is_closed2", a_file.is_closed)
...
end -- class KL_TEST_WINDOWS_INPUT_FILE
