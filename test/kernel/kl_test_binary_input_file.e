indexing
	...
	description:
		...
		"Test features of class KL_BINARY_INPUT_FILE"
...
deferred class KL_TEST_BINARY_INPUT_FILE
...
feature -- Test
	...
	test_change_name1 is
		...
		local
			a_file: KL_BINARY_INPUT_FILE
			old_file: KL_BINARY_OUTPUT_FILE
		...
		do
			...
			if out_file.is_open_write then
				...
				if a_file.is_open_read then
					a_file.read_character
					assert ("not_eof1", not a_file.end_of_file)
					assert_characters_equal ("new_line", '%N', a_file.last_character)
					a_file.read_character
					assert ("eof1", a_file.end_of_file)
					a_file.close
					assert ("is_closed2", a_file.is_closed)
