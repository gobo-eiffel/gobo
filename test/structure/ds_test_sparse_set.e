...
deferred class DS_TEST_SPARSE_SET
...
feature {NONE} -- Implementation
	...
	check_integer_set1 (a_set: DS_SPARSE_SET [INTEGER]) is
		...
		do
			...
			a_set.put_new (3)
			a_set.force_new (4)
			assert_iarray ("items1", <<1, 2, 3, 4>>, a_set.to_array)
			...
			assert_iarray ("items2", <<1, 2, 4>>, a_set.to_array)
			a_set.force_last (10)
			assert_iarray ("items3", <<1, 2, 4, 10>>, a_set.to_array)
			a_set.put (5)
			assert_iarray ("items4", <<1, 2, 5, 4, 10>>, a_set.to_array)
			a_set.remove (10)
			a_set.put (6)
			assert_iarray ("items5", <<1, 2, 5, 4, 6>>, a_set.to_array)
			a_set.remove (6)
			assert_iarray ("items6", <<1, 2, 5, 4>>, a_set.to_array)
			a_set.put (7)
			assert_iarray ("items7", <<1, 2, 5, 4, 7>>, a_set.to_array)
			a_set.wipe_out
			assert ("empty2", a_set.is_empty)
			a_set.put (8)
			assert_iarray ("items8", <<8>>, a_set.to_array)
			a_set.put (8)
			assert_iarray ("items9", <<8>>, a_set.to_array)
		end

end -- class DS_TEST_SPARSE_SET
