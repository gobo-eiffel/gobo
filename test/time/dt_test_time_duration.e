...
deferred class DT_TEST_TIME_DURATION
...
feature -- Test
	...
	test_time_duration1 is
		...
		local
			...
			t1, t2: DT_TIME
		do
			!! d1.make_precise (4, 11, 4, 543)
			!! d2.make_precise (4, 11, 4, 543)
			!! t1.make_precise (14, 23, 54, 583)
			!! t2.make_precise (18, 34, 59, 126)
			assert_equal ("time", t2, d1.time (t1))
			!! dtd.make_precise (0, 0, 0, 4, 11, 4, 543)
			assert_equal ("to_date_time_duration", dtd, d1.to_date_time_duration)
			!! d3.make_precise (8, 22, 8, 1086)
	...
	test_time_duration2 is
		...
		do
			!! d1.make_precise (4, 11, 4, 543)
			!! d2.make_precise (4, 11, 12, 754)
			!! t1.make_precise (14, 23, 54, 583)
			!! t2.make_precise (18, 35, 7, 337)
			assert_equal ("time", t2, d2.time (t1))
			!! dtd.make_precise (0, 0, 0, 4, 11, 12, 754)
			assert_equal ("to_date_time_duration", dtd, d2.to_date_time_duration)
			!! d3.make_precise (8, 22, 16, 1297)
			assert_equal ("infix_plus", d3, d1 + d2)
			!! d3.make_precise (0, 0, -8, -211)
			assert_equal ("infix_minus", d3, d1 - d2)
			!! d3.make_precise (4, 11, 12, 754)
	...
	test_time_duration3 is
		...
		do
			!! d1.make_precise (4, 11, 4, 543)
			!! d2.make_precise (2, 8, 12, 0)
			!! t1.make_precise (14, 23, 54, 583)
			!! t2.make_precise (16, 32, 6, 583)
			assert_equal ("time", t2, d2.time (t1))
			!! dtd.make (0, 0, 0, 2, 8, 12)
			assert_equal ("to_date_time_duration", dtd, d2.to_date_time_duration)
			!! d3.make_precise (6, 19, 16, 543)
			assert_equal ("infix_plus", d3, d1 + d2)
			!! d3.make_precise (2, 3, -8, 543)
			assert_equal ("infix_minus", d3, d1 - d2)
			!! d3.make (2, 8, 12)
			assert_equal ("prefix_plus", d3, + d2)
			!! d3.make (-2, -8, -12)
	...
	test_time_duration4 is
		...
		do
			!! d1.make_precise (4, 11, 4, 543)
			!! d2.make_precise (-12, 8, -23, 2)
			!! t1.make_precise (14, 23, 54, 583)
			!! t2.make_precise (2, 31, 31, 585)
			assert_equal ("time", t2, d2.time (t1))
			!! dtd.make_precise (0, 0, 0, -12, 8, -23, 2)
			assert_equal ("to_date_time_duration", dtd, d2.to_date_time_duration)
			!! d3.make_precise (-8, 19, -19, 545)
			assert_equal ("infix_plus", d3, d1 + d2)
			!! d3.make_precise (16, 3, 27, 541)
			assert_equal ("infix_minus", d3, d1 - d2)
			!! d3.make_precise (-12, 8, -23, 2)
			assert_equal ("prefix_plus", d3, + d2)
			!! d3.make_precise (12, -8, 23, -2)
			assert_equal ("prefix_minus", d3, - d2)
			assert ("infix_less1", not (d1 < d2))
	...
	test_time_duration6 is
		...
		local
			d1, d2: DT_TIME_DURATION
		do
			!! d1.make_precise (0, 0, 0, 675452089)
			!! d2.make_precise (187, 37, 32, 89)
			assert_equal ("to_canonical1", d2, d1.to_canonical)
			!! d1.make_precise_canonical (675452089)
