...
feature -- Basic tests
	...
	test_string is
		...
		do
			...
			assert_equal (format_string_empty, "", format (format_string_empty, empty_array))
			assert_equal (format_string_1, "berend", format (format_string_1, <<"berend">>))
			assert_equal (format_string_1, "", format (format_string_1, <<"">>))
			assert_equal (format_string_2, "nothing", format (format_string_2, empty_array))
	...
	test_character is
		do
			assert_equal (format_string_c_1, "c", format (format_string_c_1, << character_cell ('c') >>))
		end

	test_boolean is
		do
			assert_equal (format_string_b_2, "True", format (format_string_b_2, << boolean_cell (True) >>))
			assert_equal (format_string_b_2, "False", format (format_string_b_2, << boolean_cell (False) >>))
		end

	test_integer is
		do
			assert_equal (format_string_i_1, "0", format (format_string_i_1, << integer_cell (0) >>))
			assert_equal (format_string_i_1, "31", format (format_string_i_1, << integer_cell (31) >>))
			assert_equal (format_string_i_1, "-31", format (format_string_i_1, << integer_cell (-31) >>))
			assert_equal (format_string_i_2, " 31", format(format_string_i_2, << integer_cell (31) >>))
			assert_equal (format_string_i_3, " 31 ", format(format_string_i_3, << integer_cell (31) >>))
			assert_equal (format_string_i_4, "00031", format(format_string_i_4, << integer_cell (31) >>))
			assert_equal (format_string_i_5, "009", format(format_string_i_5, << integer_cell (9) >>))
			assert_equal (format_string_i_6, "5  ", format(format_string_i_6, << integer_cell (5) >>))
			assert_equal (format_string_i_7, "+12345678", format(format_string_i_7, << integer_cell (+12345678) >>))
			assert_equal (format_string_i_8, "12345678", format(format_string_i_8, << integer_cell (+12345678) >>))
			assert_equal (format_string_d_1, "31", format(format_string_d_1, << integer_cell (31) >>))
		end

	test_unsigned_decimal is
		do
			assert_equal (format_string_u_1, "1", format (format_string_u_1, << integer_cell (1) >>))
			assert_equal (format_string_u_2, "00031", format(format_string_u_2, << integer_cell (31) >>))
		end

	test_hex is
		do
			assert_equal (format_string_x_1, "0", format (format_string_x_1, << integer_cell (0) >>))
			assert_equal (format_string_x_1, "1", format (format_string_x_1, << integer_cell (1) >>))
			assert_equal (format_string_x_1, "a", format (format_string_x_1, << integer_cell (10) >>))
			assert_equal (format_string_X_2, "A", format (format_string_X_2, << integer_cell (10) >>))
			assert_equal (format_string_x_1, "ff", format (format_string_x_1, << integer_cell (255) >>))
			assert_equal (format_string_X_2, "FF", format (format_string_X_2, << integer_cell (255) >>))
			assert_equal (format_string_x_3, "", format (format_string_x_3, << integer_cell (0) >>))
		end

	test_octal is
		do
			assert_equal (format_string_o_1, "0", format (format_string_o_1, << integer_cell (0) >>))
			assert_equal (format_string_o_1, "1", format (format_string_o_1, << integer_cell (1) >>))
			assert_equal (format_string_o_1, "12", format (format_string_o_1, << integer_cell (10) >>))
		end

	test_floating_point is
		do
			assert_equal (format_string_e_1, "1.000000e+00", format (format_string_e_1, << double_cell (1.0) >>))
			assert_equal (format_string_e_1, "0.000000e+00", format (format_string_e_1, << double_cell (0.0) >>))
			assert_equal (format_string_e_1, "1.234568e+18", format (format_string_e_1, << double_cell (1234567890123456789.0) >>))
			assert_equal (format_string_e_2, "1.00e+01", format (format_string_e_2, << double_cell (10.000) >>))
			assert_equal (format_string_e_3, "1e+00", format (format_string_e_3, << double_cell (1.0) >>))
			assert_equal (format_string_e_3, "6e+00", format (format_string_e_3, << double_cell (5.99) >>))
			assert_equal (format_string_e_4, "   2.5e+01", format (format_string_e_4, << double_cell (25.000) >>))
			assert_equal (format_string_e_5, "2.5e+01   ", format (format_string_e_5, << double_cell (25.000) >>))
			assert_equal (format_string_E_6, "2.5E+01   ", format (format_string_E_6, << double_cell (25.000) >>))
		end

	test_fixed_point is
		do
			assert_equal (format_string_f_1, "1.000000", format (format_string_f_1, << double_cell (1.0) >>))
			assert_equal (format_string_f_1, "0.100000", format (format_string_f_1, << double_cell (0.1) >>))
			assert_equal (format_string_f_2, "1.0", format (format_string_f_2, << double_cell (1.0) >>))
			assert_equal (format_string_f_2, "-1.0", format (format_string_f_2, << double_cell (-1.0) >>))
			assert_equal (format_string_f_1, "1.000000", format (format_string_f_1, << double_cell (1.0) >>))
			assert_equal (format_string_f_3, " 3.14", format (format_string_f_3, << double_cell (3.14149) >>))
			assert_equal (format_string_f_3, " 3.15", format (format_string_f_3, << double_cell (3.14501) >>))
			assert_equal (format_string_f_4, "3", format (format_string_f_4, << double_cell (3.145) >>))
			assert_equal (format_string_f_4, "4", format (format_string_f_4, << double_cell (3.6) >>))
		end

	test_mixed_point is
		do
			assert_equal (format_string_g_1, "1.000000", format (format_string_g_1, << double_cell (1.0) >>))
			assert_equal (format_string_g_1, "0.004500", format (format_string_g_1, << double_cell (0.0045) >>))
			assert_equal (format_string_g_1, "4.500000e-05", format (format_string_g_1, << double_cell (0.000045) >>))
			assert_equal (format_string_g_1, "4.500000e-06", format (format_string_g_1, << double_cell (0.0000045) >>))
			assert_equal (format_string_g_1, "-4.500000e-06", format (format_string_g_1, << double_cell (-0.0000045) >>))
			assert_equal (format_string_g_2, "1.00", format (format_string_g_2, << double_cell (1.0) >>))
			assert_equal (format_string_g_2, "-1.00", format (format_string_g_2, << double_cell (-1.0) >>))
			assert_equal (format_string_g_2, "4.50e-06", format (format_string_g_2, << double_cell (0.0000045) >>))
			assert_equal (format_string_g_2, "-4.50e-06", format (format_string_g_2, << double_cell (-0.0000045) >>))
			assert_equal (format_string_g_3, "1.0", format (format_string_g_3, << double_cell (1.0) >>))
			assert_equal (format_string_G_4, "3.14", format (format_string_G_4, << double_cell (3.1415) >>))
			assert_equal (format_string_G_4, "31.41", format (format_string_G_4, << double_cell (31.4149) >>))
			assert_equal (format_string_G_4, "31.42", format (format_string_G_4, << double_cell (31.41501) >>))
			assert_equal (format_string_G_4, "3.14E+02", format (format_string_G_4, << double_cell (314.15) >>))
		end
	...
	test_other_examples is
		...
		do
			...
			s  := "programmer"
			s1 := "programmers"
			assert_equal (format_string_other_1,
				"       360_        -1_360       _       550_0000000550_168       _0000000168  ",
				format (format_string_other_1, << integer_cell (360), integer_cell (-1), integer_cell (360), integer_cell (360), integer_cell (360), integer_cell (360), integer_cell (360) >>))
	...
	------------------------------------------------------------
			...
			assert_equal (format_string_other_6,
				" -123  100    400 123 c true some string 3.141593e+00 4.41593451 ",
				format (format_string_other_6, << integer_cell (-123), integer_cell (256), integer_cell (256), integer_cell (123), character_cell ('c'), boolean_cell (True), "some string", double_cell (3.1415934563), double_cell (3.1415934563)>>))
	...
	------------------------------------------------------------
			assert_equal (format_string_other_10, "Hello?", format (format_string_other_10, empty_array))
