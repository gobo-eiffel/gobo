...
rem copyright: "Copyright (c) 2007-2020, Eric Bezault and others"
rem license: "MIT License"
rem date: "$Date$"
rem revision: "$Revision$"


rem "usage: install.bat [-v][-t] <c_compiler>"
...
:c_compilation
	...
	cd %BIN_DIR%
	if .%VERBOSE%. == .-v. echo Bootstraping gec...
...
:ge
	...
	if .%VERBOSE%. == .-v. echo Compiling gepp...
	%BIN_DIR%\gec%EXE% --finalize --no-benchmark %GOBO%\tool\gepp\src\system.ecf
	if .%VERBOSE%. == .-v. echo Compiling gexslt...
...
:usage
	echo usage: install.bat [-v][-t] ^<c_compiler^>
