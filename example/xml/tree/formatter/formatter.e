indexing
	description:"Simpe application that reads an XML file and outputs it again"
...
feature {ANY} -- Basic operations
	...
	process_arguments is
		...
		do
			...
			if
				Arguments.argument_count > 2
			then -- we got an output file as well
...
feature {ANY} -- checks we have to do before we can run
	...
	check_parsers is
		...
		do
			...
			if
				not fact.is_any_toe_tree_available
			then
	...
	check_in_out_files is
		...
		do
			...
			if
				not INPUT_STREAM_.is_open_read (i)
			then
			...
			then
				...
				if
					not OUTPUT_STREAM_.is_open_write (o)
				then
