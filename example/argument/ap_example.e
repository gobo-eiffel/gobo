indexing
	...
	description:
		...
		"Ficiticious application to demonstrate the use of the command line parsing library"
	
	author: "Bernd Schoeller"
	copyright: "(c) 2006 Bernd Schoeller (bernd@fams.de)"
...
feature {NONE} -- Initialization 
	...
	make is
		...
		do
			...
			-- An alternative options list is a list that will offers an
			...
			-- It is possible to inlude existing or new options into the
	...
	display_results is
		...
		do
			if alt_flag.was_found then
				print ("The command line was used with the alternative set of options.%N")
			end			

			-- It is possible to ask, how often an option was found.

			if example_flag.was_found then
				print ("The example flag was found"+example_flag.occurrences.out+" time(s).")
			end
			if example_string_option.was_found then
				print ("The example string option was passed the parameter %""+example_string_option.parameter+"%"%N")
			end
			if example_integer_option.was_found then
				print ("The example integer option was passed the parameter "+example_integer_option.parameter.out+"%N")				
			end
			if example_enumeration_option.was_found then
				print ("The example enumeration option was passed the parameter "+example_enumeration_option.parameter+"%N")
			end
			if example_boolean_option.was_found then
				if example_boolean_option.is_true then
					print ("The boolean option was set to true.%N")
				else
					print ("The boolean option was set to false.%N")					
				end
			end
			if alt_string_option.was_found then
				print ("The alternative string option was passed the parameter %""+alt_string_option.parameter+"%"%N")
			end
		end
